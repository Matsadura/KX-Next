...
/**
 * InitializeHackInBackground - Initializes the Hack object in a background thread.
 * Returns: A unique_ptr to the initialized Hack object, or nullptr on failure.
 */
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    else
    {
        ...
        if (!::IsWindowVisible(hwnd))
            ::ShowWindow(hwnd, SW_SHOW); // Make sure window is visible
    }
    ...
    while (!done)
    {
        ...
        if (done)
            break; // Exit loop immediately if WM_QUIT received
        ...
        if (isLoading)
        { // Only true if status check passed and background task is running
        ...
        { // Status check OK, Hack init OK, GUI exists
        ...
        { // Either status check failed OR Hack init failed
...
/**
 * WndProc - Window procedure to handle Windows messages.
